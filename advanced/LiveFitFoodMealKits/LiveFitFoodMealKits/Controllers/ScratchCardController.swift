...
class ScratchCardController: CommonViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    let CHANCES = [50, 10, 10, 10, 10, 10, 10]
    ...
    func showCoupon() {
        guard let coupon = self.coupon else {
            print("coupon is nil")
            txtInstruction.text = MSG_TRY_AGAIN
    ...
    func getShakeTime() -> Date? {
        if let _ = UserDefaults.standard.object(forKey: SHAKE_TIME) {
            return defaults.object(forKey: SHAKE_TIME) as? Date
        }
        return nil
    }
    ...
    func setShakeTime() {
        print("set time")
        defaults.set(Date(), forKey: SHAKE_TIME)
    }
    ...
    func validateShakeTime() {
        ...
        if calendar.component(.day, from: shakeTime) < calendar.component(.day, from: Date()) {
            ...
            self.canShake = true
            txtInstruction.text = MSG_INSTRUCTIONS
            return
        }else {
            txtInstruction.text = MSG_TRY_AGAIN
        }
    ...
    func fetchData() {
        ...
        request.predicate = NSPredicate(format: "owned_by_user == %@", mainController!.user!)
        do {
...
//        self.coupon = nil
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.coupons[indexPath.row].applied) {
            cell.textLabel!.text = "\(self.coupons[indexPath.row].code!)\t\tUSED"
        }else {
            cell.textLabel!.text = "\(self.coupons[indexPath.row].code!)\t\t\(self.coupons[indexPath.row].discount)% OFF"
        }
