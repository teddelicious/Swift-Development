...
class OrderConfirmationController: CommonViewController {
    ...
    func calculateSubtotal() {
        self.subtotal = Double(self.qty) * self.mealKit!.price
    ...
    func applyCoupon() {
        ...
        self.price = self.price * Double(coupon.discount) / 100.00
    ...
    @IBAction func onPlaceOrderPressed(_ sender: Any) {
        ...
        if let coupon = self.couponUsed as? NSManagedObject {
            ...
            newOrder.setValue(NSSet(array: [newOrderItem]), forKey: "ordered_meal_kit")
